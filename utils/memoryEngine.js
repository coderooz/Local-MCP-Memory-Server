...
export async function captureMemoryVersion(
  ...
  const version = new MemoryVersionModel({
    ...
    context_version: previousContext.version || 1,
    change_type,
...
export async function evaluateMemoryState(
  ...
  if (
    nextImportance === context.importance &&
    nextLifecycle === context.lifecycle
  ) {
    return context;
  }
...
export async function optimizeMemories(db, { project, limit = 100 } = {}) {
  ...
  for (const context of contexts) {
    ...
    if (
      next.importance !== context.importance ||
      next.lifecycle !== context.lifecycle
    ) {
      summary.changed += 1;
    }
    ...
    if (next.lifecycle === MEMORY_LIFECYCLE.ARCHIVED &&
...
export async function detectAndResolveMemoryConflicts(db, context) {
  ...
  const conflicts = candidates.filter((candidate) => {
    ...
    const sameContent =
      normalizeText(candidate.content) === normalizeText(context.content);
  ...
  for (const candidate of conflicts) {
    ...
    const loserUpdate = {
      ...
      conflictsWith: Array.from(
        new Set([...(currentLoser.conflictsWith || []), winner.id])
      ),
...
export async function getConnectedContextData(db, contextId) {
  ...
  const [actions, relatedContexts, relatedTasks, relatedIssues, versions, descriptor] =
    await Promise.all([
      ...
      relatedContextIds.length
        ? db.collection("contexts").find({ id: { $in: relatedContextIds } }).toArray()
        : [],
      (context.relatedTasks || []).length
        ? db.collection("tasks").find({ task_id: { $in: context.relatedTasks } }).toArray()
        : [],
      (context.relatedIssues || []).length
        ? db.collection("issues").find({ issue_id: { $in: context.relatedIssues } }).toArray()
        : [],
      db.collection("memory_versions")
  ...
  const relatedAgents = relatedAgentIds.length
    ? await db.collection("agents").find({
        $or: [
          { agent_id: { $in: relatedAgentIds } },
          { agent: { $in: relatedAgentIds } }
        ]
      }).toArray()
    : [];
